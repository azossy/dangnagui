...
> 임금님귀 v1.2.4 · copyright by 챠리 (challychoi@me.com)

토픽별 핫키워드 · 네티즌 의견을 **DuckDuckGo 실시간 검색**으로 수집하여  
신문기사/보도자료 스타일의 메신저 리포트를 생성하는 Windows 데스크톱 앱.  
...
1. **[dangnagui-setup-v1.2.4.exe](https://github.com/azossy/dangnagui/releases/latest)** 다운로드
...
1. **[dangnagui-v1.2.4-portable.zip](https://github.com/azossy/dangnagui/releases/latest)** 다운로드
...
- [Inno Setup 6](https://jrsoftware.org/isinfo.php) 설치 시 → `Output\dangnagui-setup-v1.2.4.exe` 자동 생성
...
├── report_engine.py      # 실시간 검색 + 언어필터 + 리포트 포맷
...
|--------|------|
| `pyperclip` | 클립보드 복사 |
| `duckduckgo-search` | 실시간 웹검색 + 토픽 사이트 탐색 + 번역 |
| `lxml` | HTML 파싱 |
| `pyinstaller` | EXE 빌드 |
